...
for sampleid in Sample:
    ...
    outfile.close()
    cmd0 = 'unset PYTHONPATH;/home/changluyuan/.pyenv/versions/3.6.2/bin/snakemake -p -s %s/Snakefile_2.cnf.py  --cluster "corvidrun-cnf 1 {threads}" -j 1000 --latency-wait 3600 --unlock' % dirname
    cmd = 'corvidrun-gpun 1 1 \'unset PYTHONPATH;nohup /home/changluyuan/.pyenv/versions/3.6.2/bin/snakemake -p -s %s/Snakefile_2.cnf.py  --cluster "corvidrun-cnf 1 {threads}" -j 1000 --latency-wait 7200 > %s.log 2>&1 &\'' %(dirname,dirname)
    print cmd0
    print cmd
    os.system(cmd0)
    os.system(cmd)
